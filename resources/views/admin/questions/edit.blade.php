<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Question in {{ $quiz->title }}</h2>
            <a
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('admin.quizzes.questions.update', ['quiz' => $quiz, 'question' => $question]) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div class="md:col-span-2">
                                ...
                                <textarea id="prompt" name="prompt" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>{{ old('prompt', $question->prompt) }}</textarea>
                            ...
                            <div>
                                ...
                                <select id="type" name="type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                    @foreach ($questionTypes as $key => $label)
                                        <option value="{{ $key }}" @selected(old('type', $question->type) === $key)>{{ $label }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input id="points" name="points" type="number" step="0.01" min="0" value="{{ old('points', $question->points) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                            ...
                            <div>
                                ...
                                <input id="order_index" name="order_index" type="number" min="1" value="{{ old('order_index', $question->order_index) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        <div class="border border-gray-200 rounded-md p-4 space-y-3">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input id="numerical_correct_answer" name="numerical_correct_answer" type="number" step="any" value="{{ old('numerical_correct_answer', $question->numerical_correct_answer) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                ...
                                <div>
                                    ...
                                    <input id="numerical_tolerance" name="numerical_tolerance" type="number" step="any" min="0" value="{{ old('numerical_tolerance', $question->numerical_tolerance ?? 0.01) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
